...
──────────────────────────────────────────────────────────────────────────────────
 ✓ zsh + Bash connected · ~/.zshrc and ~/.bash_profile and ~/.bashrc
...
- **Import:** reads the aliases you already have in `~/.zshrc`, `~/.bash_profile` and `~/.bashrc` and writes a backup of each file first. It is offered automatically on first start.
...
- **Schedules:** fixed time, sunrise, or sunset, on chosen weekdays, through macOS launchd. They also run when the TUI is closed.
...
Not included, because they only make sense for a windowed app: global hotkeys, the menu-bar icon, and autostart. An automation's hotkey set in the desktop app is kept when you edit it in the TUI.
...
| --- | --- |
| `~/.easyalias/config.json`, `aliases.zsh` | yes |
...
The TUI adds `source ~/.easyalias/aliases.zsh` to your shell files once, just like the app. It does not add the app's `easya` launcher alias.

Schedules are registered with launchd under the same labels the desktop app uses, so an automation has one schedule no matter which program saved it. When the TUI runs from Homebrew, it registers the stable `bin/` path rather than the versioned Cellar path, so schedules keep working after `brew upgrade`.
...
The data layer (`models`, `aliases`, `automations`, `session`, `timed`, `sun`, `launchd`, …) is the desktop app's backend (`mac_src/src-tauri/src`) without Tauri. The terminal interface lives in `src/tui/`.
