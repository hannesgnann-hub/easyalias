...
fn creates_edits_favorites_and_trashes_an_alias_with_the_keyboard() {
    ...
    let generated = fs::read_to_string(home.path.join(".easyalias/aliases.zsh")).unwrap();
    assert!(
        generated.contains("alias proj='cd \"$HOME/Projects/my app\"'"),
        "{}",
        generated
    );
    ...
    assert_eq!(alias.command_preview, "open \"$HOME/Projects/my app\"");
...
fn builds_and_runs_an_automation_in_one_shell_session() {
    ...
    press(&mut app, KeyCode::Down);
    type_text(&mut app, "export GREETING=hi && cd ..");
    ctrl(&mut app, 'p');
    ctrl(&mut app, 'u');
    type_text(&mut app, "1");
    ctrl(&mut app, 'n');
    type_text(&mut app, "echo $GREETING from $(basename $PWD)");
    ctrl(&mut app, 'n');
...
fn stop_ends_a_running_automation_immediately() {
    let _lock = HOME_LOCK
        ...
        .unwrap_or_else(|poisoned| poisoned.into_inner());
    let home = TemporaryHome::create();
    let mut automation = crate::tests::test_automation("slow", "Slow", "sleep 30");
