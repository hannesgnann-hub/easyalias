...
fn draw_aliases(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    let status_line = if state.shell_source_present {
        Line::from(vec![
            Span::styled(" ✓ ", theme.ok()),
            Span::styled(format!("{} connected", state.shell_name), theme.dim()),
            Span::styled(format!(" · {}", state.shell_config_file), theme.dim()),
        ])
    } else {
        Line::from(vec![
...
fn draw_settings(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    let info = vec![
        row("Aliases data", Span::raw(state.config_file.clone())),
        row("Generated file", Span::raw(state.aliases_file.clone())),
        row("Shell files", Span::raw(state.shell_config_file.clone())),
        row(
            "Shell",
            if state.shell_source_present {
                Span::styled(format!("✓ {} connected", state.shell_name), theme.ok())
            } else {
                Span::styled(
                    format!("⚠ {} not connected", state.shell_name),
                    theme.warn(),
                )
            },
        ),
...
fn draw_modal(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    match &modal {
        ...
        Modal::ShellImport { selected, checked } => {
            let candidates = &app.alias_state.import_candidates;
            ...
            let inner = open_modal(
                ...
                100,
                height,
                "Import existing aliases".to_string(),
            );
            ...
            frame.render_widget(
                Paragraph::new(format!(
                    "Found {} alias{} in your shell startup files. Imported lines are commented out in {} - a backup of each changed file is written first.",
                    candidates.len(),
                    if candidates.len() == 1 { "" } else { "es" },
                    app.alias_state.shell_config_file
                ))
                .style(theme.dim())
                .wrap(Wrap { trim: true }),
                intro,
            ...
            let rows = checklist_rows(
                ...
                candidates.iter().map(|c| {
                    (
                        checked.contains(&c.id),
                        c.name.clone(),
                        format!("{}   ({}:{})", c.command, c.source_file, c.line_number),
                    )
                }),
            );
            let table = Table::new(
...
fn draw_alias_form(frame: &mut Frame, theme: &Theme, area: Rect, form: &AliasForm) {
    ...
    if form.action() == "custom" {
        ...
        let placeholder = match form.action() {
            "navigate" | "compile_gradle" | "compile_maven" => "~/Projects/app  (Tab completes)",
            _ => "~/path/to/file  (Tab completes)",
        };
        field_row(
            frame,
            theme,
...
fn draw_schedule(frame: &mut Frame, app: &App, area: Rect, state: &ScheduleState) {
    ...
    if let Some(at) = state.entry.last_run_at {
        ...
        info.push(Line::styled(
            "macOS launchd runs it even while EasyAlias is closed.",
            theme.dim(),
        ));
    }
    frame.render_widget(
