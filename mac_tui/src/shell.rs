...
pub(crate) fn mark_import_handled(setup: &ShellSetup) -> Result<(), String> {
...
pub(crate) fn next_shell_backup_file(path: &Path) -> Result<PathBuf, String> {
    let file_name = path
        .file_name()
...
pub(crate) fn ensure_shell_file(path: &Path) -> Result<(), String> {
...
pub(crate) fn shell_config_file_display(setup: &ShellSetup) -> Result<String, String> {
    setup
        .config_files
        .iter()
        .cloned()
