...
pub(crate) const SCHEDULER_NOTE: &str = "macOS launchd runs it even while EasyAlias is closed.";
...
// that is written into aliases.zsh (same rules as the desktop app).
...
// Alias names are case-sensitive in zsh and bash.
...
pub(crate) fn about_rows(state: &AppState) -> Vec<(&'static str, String)> {
    vec![
        ...
        ("Shell files", state.shell_config_file.clone()),
    ]
}
...
pub(crate) fn import_candidates(state: &AppState) -> Vec<ImportCandidate> {
    state
        ...
        .map(|candidate| ImportCandidate {
            ...
            detail: format!(
                "{}   ({}:{})",
                candidate.command, candidate.source_file, candidate.line_number
            ),
        })
        .collect()
}
