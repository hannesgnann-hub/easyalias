...
// Safe Linux developer shortcuts compatible with both bash and zsh. They are
...
// Nine cards fill the three-column suggestion grid without making the panel
// overly tall. Additional suggestions remain reachable through pagination.
    ...
    <section class="shell settings-shell">
      ...
      <div class="settings-card">
        <div class="settings-card-head">
          <h2>Appearance</h2>
          <p>Choose how EasyAlias looks. "System" follows your desktop's light/dark setting.</p>
        </div>
  ...
  appElement.innerHTML = `
    <section class="shell automation-shell">
      <header class="topbar automation-topbar">
        <div>
          <p class="eyebrow">Linux Workflow Runner</p>
          <h1>Automations</h1>
        </div>
        <div class="topbar-actions">
      ...
      <section class="automation-summary">
        ...
        <div><span>Shell</span><strong>${escapeHtml(appState.shellName || "bash")}</strong></div>
      </section>
...
function render() {
  ...
  appElement.innerHTML = `
    <section class="shell">
      ...
      ${notice ? `<div class="message-banner notice" role="status"><span>${escapeHtml(notice)}</span><button class="message-dismiss" type="button" title="Dismiss message" aria-label="Dismiss message" data-action="dismiss-message"><i data-lucide="x"></i></button></div>` : ""}
      ${error ? `<div class="message-banner error" role="alert"><span>${escapeHtml(error)}</span><button class="message-dismiss" type="button" title="Dismiss message" aria-label="Dismiss message" data-action="dismiss-message"><i data-lucide="x"></i></button></div>` : ""}
                              ...
                              ${Array.from({ length: suggestionPageCount }, (_, index) => index + 1)
                                .map(
                                  (page) => `<button
                                    ...
                                    aria-current="${page === suggestionPage ? "page" : "false"}"
                                    data-action="suggestion-page"
                                    data-page="${page}"
                                  >${page}</button>`
                                )
  ...
  return `
    <section class="modal-layer" role="presentation">
      <form class="modal-card import-card backup-card" id="${isExport ? "backup-export-form" : "backup-import-form"}" role="dialog" aria-modal="true" aria-labelledby="backup-title">
        ...
        ${isExport ? "" : `<button class="backup-drop-zone" type="button" data-action="choose-backup-file" ${backupBusy ? "disabled" : ""}>
          <span class="backup-drop-icon" aria-hidden="true">&#8681;</span>
        ...
        ${backupCandidates.length ? `<label class="import-select-all">
        ...
        <div class="import-list" aria-label="Aliases available for ${isExport ? "export" : "import"}">
          ${backupCandidates.map((alias) => {
            const willReplace = !isExport && existingNames.has(alias.name);
            return `<label class="import-row">
              <input type="checkbox" name="backup-candidate" value="${escapeHtml(alias.id)}" ${selectedBackupIds.has(alias.id) ? "checked" : ""} ${backupBusy ? "disabled" : ""} />
              <span class="import-alias-copy">
