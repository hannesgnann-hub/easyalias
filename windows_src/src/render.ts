    ...
    <section class="shell">
      ...
      <section class="status-grid">
        <div>
          ...
          <strong>${state.appState.commandDir}</strong>
        </div>
        <div>
          <span>User PATH</span>
          <strong>${state.appState.pathConfigured ? "Connected" : "Restart terminal"}</strong>
        </div>
        <div>
          <span>Aliases</span>
        ...
        <form class="editor" id="alias-form">
          ...
          ${
            state.form.action === "custom"
              ? `<label>
                  Custom Command
                  <textarea name="customCommand" rows="4" placeholder='cd /d "%USERPROFILE%\\project" && run.bat'>${escapeHtml(state.form.customCommand)}</textarea>
                </label>`
              : ""
          }
