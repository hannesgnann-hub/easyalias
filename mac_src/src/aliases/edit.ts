  ...
  return `
    <section class="modal-layer" role="dialog" aria-modal="true" aria-labelledby="edit-title" data-dialog="edit-alias">
      <form class="modal-card" id="edit-form">
        ...
        <label>
          Command Name
          <input name="edit-name" value="${escapeHtml(state.editForm.name)}" placeholder="myproject" autocomplete="off" />
        </label>

        <label>
          Location / File / Command
          <span class="path-picker-row">
            <input name="edit-path" value="${escapeHtml(state.editForm.path)}" placeholder="~/Projects/my-app" autocomplete="off" />
