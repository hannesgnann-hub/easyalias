...
// day, running through the OS's own scheduler (launchd on macOS) so it
// self-corrects even while VS Code is already open. `lightStart`/
  ...
  appElement.innerHTML = `
    <section class="shell automation-shell">
      ...
      <form class="modal-card vscode-theme-schedule-form" id="vscode-theme-schedule-form">
        <p class="automation-intro">Runs through the operating system's own scheduler - checks every ${VSCODE_THEME_CHECK_INTERVAL_MINUTES} minutes and once at login, so VS Code corrects itself even if it was already open when the window changed.</p>
...
function renderAutomationsView() {
  ...
  createIcons({
    icons: { ArrowDown, ArrowLeft, ArrowUp, CircleStop, Clock, Clock3, FileDown, FileUp, Filter, FolderOpen, LoaderCircle, Pencil, Play, Plus, RotateCcw, Save, Search, Star, Tag, Tags, Terminal, Trash2, X },
    attrs: { "aria-hidden": "true", width: "20", height: "20", "stroke-width": "2" }
  });
  scheduleMessageDismissal();
