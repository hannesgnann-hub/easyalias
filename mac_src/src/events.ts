...
import type { AliasAction, AliasFilter } from "./types";
...
export function bindEvents() {
  ...
  document.querySelector<HTMLInputElement>('input[name="alias-search"]')?.addEventListener("input", (event) => {
    state.aliasSearchQuery = (event.target as HTMLInputElement).value;
    state.aliasPage = 1;
    refreshAliasResults();
  });

  document.querySelector<HTMLSelectElement>('select[name="alias-filter"]')?.addEventListener("change", (event) => {
    state.aliasFilter = (event.target as HTMLSelectElement).value as AliasFilter;
    state.aliasPage = 1;
    refreshAliasResults();
  });
  ...
  document.querySelector<HTMLInputElement>('input[name="import-all"]')?.addEventListener("change", (event) => {
    ...
    state.selectedImportIds = checked
  ...
  document.querySelectorAll<HTMLInputElement>('input[name="import-candidate"]').forEach((checkbox) => {
    checkbox.addEventListener("change", () => {
      if (checkbox.checked) {
        state.selectedImportIds.add(checkbox.value);
      } else {
        state.selectedImportIds.delete(checkbox.value);
      }
      render();
    });
  });
  ...
  document.querySelectorAll<HTMLInputElement>('input[name="backup-candidate"]').forEach((checkbox) => {
    checkbox.addEventListener("change", () => {
      if (checkbox.checked) {
        state.selectedBackupIds.add(checkbox.value);
      } else {
        state.selectedBackupIds.delete(checkbox.value);
      }
      syncBackupSelectionControls();
    });
  });
