...
impl RunState {
    ...
    pub(crate) fn stop(&mut self) {
        ...
        let keep: Vec<u32> = self
            ...
            .map(|pids| pids.clone())
            .unwrap_or_default();
        for child in platform::child_pids(shell) {
            if !keep.contains(&child) {
                platform::kill_tree(child);
            }
        }
        platform::kill(shell);
    }
...
fn run_worker(
    ...
    let shell_pid = session.child.id();
    pid.store(shell_pid, Ordering::SeqCst);
        ...
        match execute_in_session(&mut session, &step.command, background) {
            Ok(result) => {
                let output = [result.stdout.trim(), result.stderr.trim()]
                    ...
                    .join("\n");
                if background {
                    // Remember what this step started so Stop leaves it running.
                    // Not every shell reports the job's PID ($! in zsh/bash, nothing
                    // in cmd.exe), so the shell's current children are kept too.
                    if let Ok(mut pids) = background_pids.lock() {
                        pids.extend(result.process_id);
                        pids.extend(platform::child_pids(shell_pid));
                    }
