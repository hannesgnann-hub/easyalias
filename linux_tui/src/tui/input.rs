...
impl TextInput {
    ...
    pub(crate) fn complete_path(&mut self) -> Vec<String> {
        ...
        // Windows paths may use either separator; elsewhere "\\" is a normal character.
        let separator = |c: char| c == '/' || (cfg!(windows) && c == '\\');
        let dir_suffix = if cfg!(windows) && typed.contains('\\') { "\\" } else { "/" };
        let (dir_part, prefix) = match typed.rfind(separator) {
