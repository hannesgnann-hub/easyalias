//! In-app tutorial (Windows), adapted from the desktop app's tutorial for the terminal.
...
pub(crate) const TOPICS: &[HelpTopic] = &[
    HelpTopic {
        ...
        steps: &[
            (
                "What an alias is",
                "An alias is a short name for something you would otherwise type in full - a folder to jump to, a file to open, or a command to run.\n\nEasyAlias stores your aliases as structured data and turns each one into a tiny command, so typing `myproj` in a terminal can mean `cd /d \"%USERPROFILE%\\Projects\\my-project\"`.\n\nIt only manages its own folder of .cmd files - nothing else is changed.",
            ),
            (
                "Create one",
            ...
            (
                "Import & backup",
                "- `i` looks for simple .cmd/.bat shortcut files you already have in your own PATH folders and imports the ones you pick. A copy of each file is kept in a backup folder first.\n- `b` exports a portable JSON backup, `B` imports one - useful to move aliases between machines.\n- `t` opens the 30-day Trash: `r` restores, `d` deletes forever, `E` empties it.",
            ),
            (
                "Using them in the terminal",
                "EasyAlias writes one .cmd file per alias into ~\\.easyalias\\bin and adds that folder to your user PATH once.\n\nOpen a new terminal window (cmd, PowerShell or Windows Terminal) so the updated PATH is picked up, then type your alias like any command. `where myalias` shows where it resolves.\n\nThe data is shared with the EasyAlias desktop app, so you can use both side by side.",
            ),
        ],
    },
    HelpTopic {
        ...
        steps: &[
            ...
            (
                "Build one",
                "Press `n`. Give it a name and a working directory (Tab completes paths), optionally a group, then add steps:\n\n- Ctrl+N - add a Command step (cmd.exe syntax). Ctrl+T switches whether the next step waits for it to finish or starts as soon as it launched (background - handy for dev servers).\n- Ctrl+P - add a Wait (pause) step, 1 second to 24 hours.\n- Shift+↑ ↓ moves a step, Ctrl+X removes it.\n\nIf the working directory points at a file, the run uses the folder that contains it. Ctrl+S saves.",
            ),
            (
                "One shell session",
                "Every command step in a run shares one cmd.exe session started in the working directory.\n\nSo a `cd` or a `set` variable in one step is still in effect for every step after it - exactly like a real terminal.",
            ),
            (
                "Run it",
            ...
            (
                "Schedule it",
                "Press `c` on an automation to schedule it. Pick a trigger with ← →:\n\n- Time - a fixed HH:MM.\n- Sunrise / Sunset - that day's real event for the region you choose, recomputed daily.\n\nOptionally limit it to certain weekdays (← → and Space, or 1-7). EasyAlias hands the schedule to Windows Task Scheduler, so it fires even when EasyAlias is closed. The list shows the last run's result. Ctrl+D removes a schedule.",
            ),
        ],
    },
    HelpTopic {
