...
impl AliasForm {
    ...
    pub(crate) fn preview(&self) -> String {
        platform::preview_command(self.action(), &self.path.value, &self.custom.value)
    }
}
...
impl App {
    pub(crate) fn load() -> Self {
        ...
        if !candidates.is_empty() {
            let checked = candidates.into_iter().map(|c| c.id).collect();
    ...
    pub(crate) fn submit_alias_form(&mut self, form: &AliasForm) -> Result<(), String> {
        ...
        let duplicate = self.alias_state.aliases.iter().any(|alias| {
            platform::same_alias_name(&alias.name, &name)
                && Some(&alias.id) != form.editing_id.as_ref()
        });
        if duplicate {
        ...
        self.notice(platform::saved_notice(&self.alias_state));
        Ok(())
    }
    ...
    pub(crate) fn use_suggestion(&mut self, index: usize) {
        ...
        aliases.push(AliasEntry {
            ...
            command_preview: platform::preview_command(
                suggestion.action,
                suggestion.path,
    ...
    pub(crate) fn open_shell_import(&mut self) {
        match platform::scan_import() {
            Ok(state) => {
                let candidates = platform::import_candidates(&state);
                self.alias_state = state;
                if candidates.is_empty() {
                    self.notice(platform::nothing_to_import(&self.alias_state));
                } else {
    ...
    pub(crate) fn import_shell(&mut self, checked: &HashSet<String>) -> Result<(), String> {
        ...
        let (state, notice) = platform::import_selected(checked.iter().cloned().collect())?;
        self.alias_state = AppState {
            import_candidates: Vec::new(),
            ..state
        };
    ...
    pub(crate) fn open_url(&mut self, url: &str) {
        if platform::open_url(url) {
            ...
            self.error(format!("Link could not be opened: {}", url));
        }
    }
}
